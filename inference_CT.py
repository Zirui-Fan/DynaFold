...
from src.utils.protein_items import collect_protein_pair
...
def build_parser():
    p = argparse.ArgumentParser("Latent diffusion sampler (trajectory between two structures)")
    p.add_argument("--pdb0", type=Path, required=True, help="Path to the first PDB file (initial structure)")
    p.add_argument("--pdbT", type=Path, required=True, help="Path to the second PDB file (final structure)")
    ...
    p.add_argument("--scaling", type=float, default=15.0)
    p.add_argument("--num_steps", type=int, default=100)
    p.add_argument("--T", type=int, default=201, help="trajectory length")
...
def main():
    ...
    protein = collect_protein_pair(args, esm_model, batch_converter, structure_encoder, device)
    LOGGER.info("Loaded protein pair: %s", protein.id)

    del esm_model, batch_converter
    del structure_encoder
    torch.cuda.empty_cache()

    edm_arch_params: Tuple[int, int, int, int, int, int] = (args.latent_dim, 384, 1280, 6, 768, 6)

    LOGGER.info("Loading EDM and VAE …")
    edm_net = LatentEDM(*edm_arch_params, use_temporal_attn=True, use_condition_frame=True).to(device)
    ...
    with torch.inference_mode():
        ...
        mu_traj[0, -1] = protein.muT.to(device) / args.scaling
        atom_mask = protein.mask.to(device) if protein.mask is not None else torch.ones(seq_len, 14, device=device)

        for rep in range(1, args.num_repeat + 1):
            t_edm_start = time.time()
            noise = torch.randn_like(mu_traj) * args.sigma_max

            latent_traj = edm_sampler(
                edm_net,
                noise,
                mu_traj,
                aa_indices,
            ...
            pdb_output = args.out_dir / f"{protein.id}_T{args.T}_R{rep}.pdb"
